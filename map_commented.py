...
# Importing the Kivy packages (to make the map)
...
# Importing the Dqn object from our AI in AI_Saurabh.py
from AI_Saurabh import Dqn      #Deep Q-Network (the brain of the car)
...
brain = Dqn(5,3,0.9) # 5 sensors/states, 3 is no. of actions (ie. go staright, left, right), gama = 0.9
action2rotation = [0,20,-20] #actions are encoded in it. action = 0 => no rotation ie. go straight, action = 1 => rotate 20 degres right, action = 2 => rotate -20 degress right ie. left
last_reward = 0 # initializing the last reward ie. if car goes to sand then bad reward, if on road then good reward.
...
def init():
    ...
    global goal_x # x-coordinate of the goal (where the car has to go, that is the airport or the downtown) ie. upper-left corner in our map case is airport, down-right corner is downtown
    global goal_y # y-coordinate of the goal (where the car has to go, that is the airport or the downtown)
    sand = np.zeros((longueur,largeur)) # initializing the sand array with only zeros. largeur is y-coordinate distance & longueur is x-coordinate distance
...
# Initializing the last distance ie. distance between car and the goal
last_distance = 0
...
class Car(Widget):
    ...
    signal1 = NumericProperty(0) # initializing the signal received by sensor 1 ie. density of sand around sensor 1
    signal2 = NumericProperty(0) # initializing the signal received by sensor 2 ie. density of sand around sensor 2
    signal3 = NumericProperty(0) # initializing the signal received by sensor 3 ie. density of sand around sensor 3
    ...
    def move(self, rotation):
        ...
        self.sensor1 = Vector(30, 0).rotate(self.angle) + self.pos # updating the position of sensor 1.   30 is the distance between car and the sensor
...
class Game(Widget):
    ...
    def update(self, dt): # the big update function that updates everything that needs to be updated at each discrete time t when reaching a new state (getting new signals from the sensors)
        ...
        last_signal = [self.car.signal1, self.car.signal2, self.car.signal3, orientation, -orientation] # our input state vector to neural network, composed of the three signals received by the three sensors, plus the orientation and -orientation
        ...
        rotation = action2rotation[action] # converting the action played (0, 1 or 2) into the rotation angle (0, 20 or -20 degrees)
        ...
        if sand[int(self.car.x),int(self.car.y)] > 0: # if the car is on the sand
            ...
            last_reward = -1 # and reward = -1 worst reward
        else: # otherwise
